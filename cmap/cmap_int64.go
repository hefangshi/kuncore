...
func (cm *CMap) Set(key string, val int64) {
	h := hasher(key)
...
func (cm *CMap) DeleteAndGet(key string) int64 {
	h := hasher(key)
...
func (cm *CMap) Update(key string, fn func(oldval int64) (newval int64)) {
	h := hasher(key)
	cm.shards[h&uint32(len(cm.shards)-1)].Update(key, fn)
}

// Swap is the equivalent of `oldVal, map[key] = map[key], newVal`.
func (cm *CMap) Swap(key string, val int64) int64 {
	h := hasher(key)
...
func (cm *CMap) ForEach(fn func(key string, val int64) bool) bool {
...
func (cm *CMap) ForEachLocked(fn func(key string, val int64) bool) bool {
	for _, lm := range cm.shards {
...
// NewLMapSize is the equivalent of `m := make(map[string]int64, cap)`
func NewLMapSize(cap int) *LMap {
	return &LMap{
		m: make(map[string]int64, cap),
		l: new(sync.RWMutex),
	}
}

// Set is the equivalent of `map[key] = val`.
func (lm *LMap) Set(key string, v int64) {
	lm.l.Lock()
	lm.m[key] = v
	lm.l.Unlock()
}
...
// SetIfNotExists will only assign val to key if it wasn't already set.
// Use `Update` if you need more logic.
func (lm *LMap) SetIfNotExists(key string, val int64) (set bool) {
	lm.l.Lock()
	if _, ok := lm.m[key]; !ok {
...
// Get is the equivalent of `val := map[key]`.
func (lm *LMap) Get(key string) (v int64) {
	lm.l.RLock()
	v = lm.m[key]
	lm.l.RUnlock()
	return
...
// GetOK is the equivalent of `val, ok := map[key]`.
func (lm *LMap) GetOK(key string) (v int64, ok bool) {
	lm.l.RLock()
	v, ok = lm.m[key]
	lm.l.RUnlock()
	return
...
// DeleteAndGet is the equivalent of `oldVal := map[key]; delete(map, key)`.
func (lm *LMap) DeleteAndGet(key string) (v int64) {
	lm.l.Lock()
	v = lm.m[key]
	delete(lm.m, key)
	lm.l.Unlock()
...
// The shard containing the key will be locked, it is NOT safe to call other cmap funcs inside `fn`.
func (lm *LMap) Update(key string, fn func(oldVal int64) (newVal int64)) {
	lm.l.Lock()
...
// Swap is the equivalent of `oldVal, map[key] = map[key], newVal`.
func (lm *LMap) Swap(key string, newV int64) (oldV int64) {
	lm.l.Lock()
	oldV = lm.m[key]
	lm.m[key] = newV
	lm.l.Unlock()
...
// It **is** safe to modify the map while using this iterator, however it uses more memory and is slightly slower.
func (lm *LMap) ForEach(keys []string, fn func(key string, val int64) bool) bool {
	lm.l.RLock()
	for key := range lm.m {
...
// You can break early by returning false
// It is **NOT* safe to modify the map while using this iterator.
func (lm *LMap) ForEachLocked(fn func(key string, val int64) bool) bool {
	lm.l.RLock()
	defer lm.l.RUnlock()
